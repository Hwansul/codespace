 ...
 * It’s a string, using a {KEY}={VALUE}; format.
...
/**
 * Cookies can also include several optional settings,
 * most using a key=value format, separated by a semicolon (;).
 *
...
/**
 * If you do not set either a max-age or expires value,
 * cookies can potentially remain in the browser indefinitely.
 *
 * As a best practice, you should generally always set one of the two values.
 * You can use the document.cookie property to get the cookies for a site.
 *
 * However, all of the cookies for a site are stored as a single string,
 * which makes getting the value of one of them a bit tedious.
 *
...
/**
 * To delete a cookie, you set it again with a max-age of 0
 * or an expires date to the current time or sooner.
 * The cookie value can be anything.
 *
